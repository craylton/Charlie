...
namespace TSCP_Sharp_unsafe
{
    public partial class TSCP
    {
        ...
        long[] color = new long [64];  /* LIGHT, DARK, or EMPTY */
        long[] piece = new long[64];  /* PAWN, KNIGHT, BISHOP, ROOK, QUEEN, KING, or EMPTY */
        long side;  /* the side to move */
        long xside;  /* the side not to move */
        long castle; // a bitfield with the castle permissions. if 1 is set,
                    //white can still castle kingside. 2 is white queenside.
				    // 4 is black kingside. 8 is black queenside. */
        long ep;  // the en passant square. if white moves e2e4, the en passant
                 // square is set to e3, because that's where a pawn would move
			     // in an en passant capture */
        long fifty; // the number of moves since a capture or pawn move, used
                   // to handle the fifty-move-draw rule */
        long hash;  // a (more or less) unique number that corresponds to the
                   // position */
        long ply;  // the number of half-moves (ply) since the
                  // root of the search tree */
        long hply; // h for history; the number of ply since the beginning
                  //of the game */
        ...
        long[] first_move = new long[MAX_PLY];

        /* the history heuristic array (used for move ordering) */
        long[] history = new long[64 * 64];
   ...
   searching max_depth ply. */
        ...
        long[] pv_length = new long [MAX_PLY];
        bool follow_pv;

        /* random numbers used to compute hash; see set_hash() in board.c */
        long[] hash_piece = new long[2*6*64];  /* indexed by piece [color][type][square] */
        long hash_side;
        long[] hash_ep = new long[64];
