...
2 = xboard format output */
      void think(long output)
      {
          ...
          for (i = 1; i <= max_depth; ++i)
          {
              ...
              {
                  for (j = 0; j < pvl[0]; ++j)
                      Console.Write(" {0}", move_str(pvptr[0 * MAX_PLY + j].b));
                  Console.WriteLine();
                  Console.Out.Flush();
              }
              if (x > 9000 || x < -9000)
      ...
      long search(long alpha, long beta, long depth, bool null_move)
      {
          long i, j, x;
          bool c, f;
          long  nullmat;
          ...
          for (i = fm[ply]; i < fm[ply + 1]; ++i)
          {
              ...
              if (x > alpha)
              {
                  ...
                  //   search it */
                  histo[(long)gen_dat[i].m.b.from* 64 + (long)gen_dat[i].m.b.to] += depth;
                  if (x >= beta)
                      return beta;
                  alpha = x;
...
so the static evaluation function will work. */
      ...
      long quiesce(long alpha, long beta)
      {
          long i, j, x;
