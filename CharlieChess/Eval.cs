...
namespace TSCP_Sharp_unsafe
{
    ...
    {
        const long DOUBLED_PAWN_PENALTY      = 10;
        const long ISOLATED_PAWN_PENALTY	    = 20;
        const long BACKWARDS_PAWN_PENALTY    = 8;
        const long PASSED_PAWN_BONUS	        = 20;
        const long ROOK_SEMI_OPEN_FILE_BONUS	= 10;
        const long ROOK_OPEN_FILE_BONUS      = 15;
        const long ROOK_ON_SEVENTH_BONUS     = 20;
        const long IL = LIGHT * 10;
        const long ID = DARK * 10; 
        /* the values of the pieces */
        long[] piece_value= new long[6] {
        ...
        long[] king_endgame_pcsq=new long[64]{
        ...
        long[] pawn_rank = new long[2 * 10];

        long[] piece_mat = new long[2];  /* the value of a side's pieces */
        long[] pawn_mat = new long[2];  /* the value of a side's pawns */
        long[] score = new long[2];  /* each side's scr */
        long eval()
        {
          long i;
	      long f;  /* file */
          ...
          for (i = 0; i < 64; ++i)
          {
              ...
              {  
                  ...
                  {
                      case PAWN:
                        scr[LIGHT] += eval_light_pawn(i, i>>3);
                          break;
                      case KNIGHT:
        ...
        long eval_light_pawn(long sq, long row)
        {
            long r;  /* the value to return */
            long f;  /* the pawn's file */
        ...
        long eval_dark_pawn(long sq, long row)
        {
            long r;  /* the value to return */
            long f;  /* the pawn's file */
